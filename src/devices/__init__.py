...
class Device:
    vendor_id: int = config.get('device_defaults.vendor_id')
    product_id: int = None
    control_interface: int = None
    report_ids: tuple[int] = ()
    unknown_group_indexes: list[tuple[int]] = []
    read_length: int = None
    hid_read_type: int = config.get('hid_defaults.hid_read_type')
    hid_read_request: int = config.get('hid_defaults.hid_read_request')
    hid_read_index: int = None
    hid_write_type: int = config.get('hid_defaults.hid_write_type')
    hid_write_request: int = config.get('hid_defaults.hid_write_request')
    hid_write_index: int = None
    hid_timeout: int = config.get('hid_defaults.hid_timeout')
...
def load_device(name: str, device_type: str = 'mouse') -> Device:
    module = __import__('devices.' + device_type + '.' + name, globals(), locals(), [name])
